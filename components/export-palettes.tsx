...
import { FileText, Link, Image, FileImage } from "lucide-react";
import React, { useCallback } from "react";
import { toast, useToast } from "./ui/use-toast";
import { usePathname } from "next/navigation";
import { toPng, toJpeg, toBlob, toPixelData, toSvg } from "html-to-image";

export default function ExportPalettes({
  setOpen,
  handleExportPdf,
  targetRef,
}: {
  ...
  const handleExportSVG = useCallback(() => {
    ...
    toSvg(targetRef.current, { cacheBust: true })
      .then((dataUrl) => {
        const link = document.createElement("a");
  ...
  return (
    <div className="flex items-center ">
      ...
      >
        <Link className="w-[32px] mx-auto  mt-[10px] mb-5" />
      ...
      >
        ...
        <p className="uppercase font-medium">SVG</p>
      </div>
    </div>
  );
}
